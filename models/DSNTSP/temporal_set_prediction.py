...
class temporal_set_prediction(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph, nodes_feature: torch.Tensor, edges_weight: torch.Tensor,
                lengths: torch.Tensor, nodes: torch.Tensor, users_frequency: torch.Tensor, med_level: torch.Tensor):
        """
        ...
        nodes_output = self.aggregate_nodes_temporal_feature(graph, lengths, nodes_output, med_level)
        #print('****** nodes_output: ', nodes)

        # (batch_size, items_total, item_embed_dim)
        nodes_output = self.global_gated_update(graph, nodes, nodes_output)

        return nodes_output.squeeze(dim=0)
