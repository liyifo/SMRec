...
class aggregate_nodes_temporal_feature(nn.Module):
    ...
    def forward(self, graph, lengths, nodes_output, med_level):
        """
        ...
        for num_nodes, length in zip(nums_nodes, lengths):
            ...
            weights = med_level.unsqueeze(dim=1)
            # print('***output_node_features***', output_node_features.shape, self.Wq.weight.shape, weights.shape)
